...
class AiVoic3 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Obx(() {
        ...
        final isQuestCompleted =
            controller.aiCallState.value == AiCallState.questCompleted;
        return Container(
          decoration: BoxDecoration(
            image: DecorationImage(
              image: AssetImage(
                isQuestCompleted
                    ? 'assets/images/quest_complate.png'
                    : isEndingSoon
                    ? 'assets/images/Popup_Share how you feel.png'
                    : 'assets/svg_images/popup_scree_liner.png',
              ),
              fit: BoxFit.cover,
            ),
          ),
          child: SafeArea(
            child: Stack(
              children: [
                    ...
                    children: [
                      ...
                      Obx(() {
                        if (controller.aiCallState.value ==
                            ...
                            controller.aiCallState.value ==
                                AiCallState.questCompleted) {
                ...
                // Status Overlays - Call Ending Soon banner from top
                Obx(() {
                  final state = controller.aiCallState.value;
                  if (state == AiCallState.endingSoon) {
                    return Positioned(
                      top: 10,
                      left: 10,
                      right: 10,
                      child: _buildCallEndingSoonBanner(controller),
                    );
                  } else if (state == AiCallState.minutesAdded ||
                      state == AiCallState.muted ||
                      state == AiCallState.networkError) {
                    return Positioned(
                      top: 0,
                      left: 0,
                      right: 0,
  ...
  /// New "Call ending soon!" banner that appears from top with 10px margins
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () {
                    // Add 10 minutes logic
                    controller.aiCallState.value = AiCallState.minutesAdded;
                    // Reset back after 3 seconds
                    Future.delayed(const Duration(seconds: 3), () {
                      if (controller.aiCallState.value ==
                          AiCallState.minutesAdded) {
                        controller.aiCallState.value = AiCallState.ongoing;
                      }
                    });
                  },
                  child: Container(
  ...
  Widget _buildTitle(VoiceCheckController controller) {
    final isQuestCompleted =
        controller.aiCallState.value == AiCallState.questCompleted;
    return FittedBox(
      fit: BoxFit.scaleDown,
      child: Row(
        ...
        children: [
          Text(
            isQuestCompleted ? 'Answer emails' : title,
            style: TextStyle(
              fontFamily: 'Wosker',
              fontSize: 28.sp,
              fontWeight: FontWeight.w400,
  ...
  Widget _buildVoiceAvatar(VoiceCheckController controller) {
    return Obx(() {
      ...
      // When quest completed, show only the image and text without background/ring
      if (isQuestCompleted) {
        return Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                image: DecorationImage(
                  image: AssetImage("assets/images/quested_complate_icons.png"),
                  fit: BoxFit.contain,
                ),
              ),
            ),
...
class RingPainter extends CustomPainter {
  ...
  RingPainter(this.progress, {this.isEndingSoon = false});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final progressPaint = Paint()
      ..shader = LinearGradient(
        colors: isEndingSoon
            ? [Color(0xFFFF8C00), Color(0xFFFFAA33)] // Orange gradient
            : [Color(0xFF3D87F5), Color(0xFF68A2F9)], // Blue gradient
