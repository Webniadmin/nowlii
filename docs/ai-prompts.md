...
This set also serves **every non-English Realtime voice call** — `_realtime_instructions`
only uses `_REALTIME_PERSONA_EN` for `en` and falls through to `_build_system_prompt` for
`de` and `es`. So the three `"neutral"` prompts below are both the text chat *and* the
German and Spanish spoken calls.

⚠️ **The 15 dead templates still carry the bug the neutral ones were fixed for**
(2026-09-07). They instruct exactly the mirror-and-question loop that made calls read as the
companion repeating the user back: `"sad"` says *"Acknowledge exactly what they said —
reflect it back"*, `"angry"` says *"validate them completely"* and hands over stock phrases
to use verbatim (*"that's so frustrating"*, *"honestly, I'd be annoyed too"*), `"anxious"`
says *"Acknowledge exactly what's worrying them"* and *"speak slowly"*, `"confused"` says to
check in with *"does that make sense?"*. **Re-enabling per-message emotion detection without
rewriting all fifteen brings the complaint straight back.** Match the frame of the `neutral`
prompts if you revive them.
